...
import { AddStudent, AddStudents } from "@/actions/college/AddStudents";
import { useFormState } from "react-dom";
import { useEffect, useState } from "react";
import { toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";
import InstructionsBeforeCollegeUploadStudentsModal from "@/ui/components/modals/Modal";
...
export default function AddStudentComp() {
    ...
    useEffect(() => {
        if (bulkUploadState?.status === 200) {
            ...
            toast.error(bulkUploadState?.message, {
                position: "top-right",
                autoClose: 1200,
                onClose: () => {
                    document.getElementById("addStudentsForm").reset();
